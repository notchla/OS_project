...
void initASL(){
    ...
    for (int i = 0; i < DEV_PER_INT; i++){
        INIT_LIST_HEAD(&(semDevice.disk[i].s_procQ));
        semDevice.disk[i].s_key = &semDev[i];
        INIT_LIST_HEAD(&(semDevice.tape[i].s_procQ));
        semDevice.tape[i].s_key = &semDev[i + DEV_PER_INT];
        INIT_LIST_HEAD(&(semDevice.network[i].s_procQ));
        semDevice.network[i].s_key = &semDev[i + DEV_PER_INT*2];
        INIT_LIST_HEAD(&(semDevice.printer[i].s_procQ));
        semDevice.printer[i].s_key = &semDev[i + DEV_PER_INT*3];
        INIT_LIST_HEAD(&(semDevice.terminalR[i].s_procQ));
        semDevice.terminalR[i].s_key = &semDev[i + DEV_PER_INT*4];
        INIT_LIST_HEAD(&(semDevice.terminalT[i].s_procQ));
        semDevice.terminalT[i].s_key = &semDev[i + DEV_PER_INT*5];
    }
    for (int i = 0; i < DEV_PER_INT; i++){
        insertSem(&(semDevice.disk[i]));
        insertSem(&(semDevice.tape[i]));
        insertSem(&(semDevice.network[i]));
        insertSem(&(semDevice.printer[i]));
        insertSem(&(semDevice.terminalR[i]));
        insertSem(&(semDevice.terminalT[i]));
    }
}
